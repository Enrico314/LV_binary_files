...
class LV_fd:
    ...
    def _set_dtypes(self):
        ...
        self.LVtimestamp = np.dtype(
            [("seconds", self.LVint64), ("fraction", self.LVuint64)]
        )
    ...
    def write_string(self, val):
        ...
        """
        for v in val:
            temp = np.fromstring(v.encode(encoding=self.encoding), dtype=self.LVstring1)
    ...
    def write_path(self, val):
        ...
        """
        temp = np.fromstring(
            "PTH0".encode(encoding=self.encoding), dtype=self.LVstring1
        )
        ...
        for chars in comps:
            ...
            temp = np.fromstring(
                chars.encode(encoding=self.encoding), dtype=self.LVstring1
            )
            self.write_numeric(temp)
...
class LV_TDMS(LV_fd):
    ...
    def read_LeadIn(self):
        ...
        self._set_dtypes()
        self.TDSm_tag = (
            self.read_numeric(self.LVstring1, c=4).tobytes().decode(self.encoding)
        )
